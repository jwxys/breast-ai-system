...
请告诉我需要执行的操作。""",
    ...
    async def _model_inference(self, session: ChatSession) -> str:
        """使用模型推理（Kimi API）"""
        from app.services.kimi_service import get_kimi_service, KIMI_SYSTEM_PROMPT
...
请问有什么可以帮您？"""
    ...
    async def _cmd_create_patient(
        ...
        """创建患者"""
        from app.patient.schemas.patient_schema import PatientCreate
    ...
    async def _cmd_update_patient(
        ...
        from app.patient.schemas.patient_schema import PatientUpdate
    ...
    async def _cmd_create_ultrasound(
        ...
        """创建超声检查"""
        from app.ultrasound.schemas.ultrasound_schema import UltrasoundCreate
    ...
    async def _cmd_schedule_followup(
        ...
        """安排随访"""
        from app.visit.schemas.visit_schema import VisitCreate
    ...
    async def _cmd_export_data(
        ...
        if export_type == "patients":
            result = await export_service.export_patients(
                format=format,
                filters=params,
            )
        elif export_type == "reports":
            result = await export_service.export_reports(
                format=format,
                date_from=params.get("date_from"),
                date_to=params.get("date_to"),
            )
        elif export_type == "statistics":
            result = await export_service.export_statistics(
            ...
            return {
                ...
                "supported_types": ["patients", "reports", "statistics"],
            }
        
        return {
            "success": True,
            "message": f"数据导出完成，共导出 {result.get('record_count', 0)} 条记录",
            "filename": result.get('filename'),
            "filepath": result.get('filepath'),
            "download_url": export_service.get_export_file_url(result.get('filepath', '')),
        }
